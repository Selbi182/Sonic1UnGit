...
; ---------------------------------------------------------------------------
orb_circledir:	equ objoff_2E		; circling direction for spikeballs (1 clockwise, -1 counter clockwise)
orb_balldata:	equ objoff_2F		; ball data array (0: number of not-fired spikeballs // 1-4: RAM indices for spikeballs)
...
.loopSpikeBalls:
		...
		move.w	a1,(a2)+				; store RAM index for spikeball

		move.l	#Orb_CircleSpikeball,obID(a1)		; set to Orb_CircleSpikeball routine
...
.finishSpikeBalls:
	...
	.dir:	move.b	d0,orb_circledir(a0)			; set circling direction
		...
		move.l	#Orb_CheckSonic,d0			; subtype 0 (LZ)
		tst.b	obSubtype(a0)				; is subtype non-zero?
		beq.s	.setType				; if not, branch
		move.l	#Orb_DisplayAndMove,d0			; subtype 2 (SLZ)
	.setType:
		move.l	d0,obID(a0)				; set chosen type
...
Orb_DisplayNoMove:
		out_of_range_with_y_check.s	.deleteWithSpikeballs,obX(a0),obY(a0) ; has Orbinaut gone offscreen? if yes, branch
		DisplaySprite
...
.deleteWithSpikeballs:
	...
	.loopDeleteBalls:
		...
		movea.w	(a2)+,a1				; copy to a1 as input for DeleteChild
...
Orb_CircleSpikeball: ; Routine 6
		...
		cmpi.l	#Map_Orb,obMap(a1)			; has parent Orbinaut object been deleted? (dirty workaround using map)
		...
		bne.s	.circleBalls				; if not, branch
		move.l	#Orb_FiredSpikeball,obID(a0)		; set spikeball to Orb_FiredSpikeball
		subq.b	#1,orb_balldata(a1)			; decrement number of remaining non-fired spikeballs
		bne.s	.fireSpikeball				; if ammo is left, branch
		move.l	#Orb_DisplayAndMove,obID(a1)		; once ammo is out, advance parent Orbinaut to Orb_DisplayAndMove to make it start moving
