...
BossSpikeball_Main:	; Routine 0
		...
		move.w	#spr_prio4,obPriority(a0)			; set render priority (on the lower side)
...
.findBoss:
		...
		bra.s	BossSpikeball_CheckPhysics ; no boss found, branch
...
; check left side of Eggman's collision box in comparison to right side of ball
		...
		blo.s	BossSpikeball_CheckPhysics		; if not, branch
...
; check top side of ball's collision box in comparison to bottom side of Eggman
		...
		bne.s	BossSpikeball_CheckPhysics		; if there are more hits, branch
		bset	#7,obStatus(a1)				; set boss flag to defeated
		clr.w	obVelX(a0)				; stop moving
		clr.w	obVelY(a0)

; loc_18F38:
BossSpikeball_CheckPhysics:
