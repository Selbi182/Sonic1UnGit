	; ASM68K compatibility
	pusho		; back up compiler options
	opt	l+,ws+	; set compiler options to allow '.' as local label symbol; allow whitespace in macros
...
; ---------------------------------------------------------------------------
	if ~def(DMAfunctions_defined)
		DMAfunctions_defined: equ 1
	endif
; ---------------------------------------------------------------------------
	if ~def(QueueStaticDMA_defined)
		QueueStaticDMA_defined: equ 1
...
QueueStaticDMA macro src,length,dest
	if ((\src)&1)<>0
		inform 3, "DMA queued from odd source $\src!"
	endif
	if ((\length)&1)<>0
	...
	move.b	#(((\length>>1)&$7FFF)>>8)&$FF,DMAEntry.SizeH(a1)		; Write top byte of size/2
	move.l	#((((\length>>1)&$7FFF)&$FF)<<24)|((\src>>1)&$7FFFFF),d0	; Set d0 to bottom byte of size/2 and the low 3 bytes of source/2
...
; ===========================================================================
	...
	; ASM68K compatibility
	popo	; restore buffered compiler options
