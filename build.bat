...
"build_tools\asm68k.exe" /k /m /p /o ae-,c+,l+,ws+,v+,op+,os+,ow+,oz+,oaq+,osq+,omq+ sonic1.asm, s1built.bin, build_tools\s1built.sym, sonic.lst > sonic.log

REM // Still print redirected log output to console (Batch doesn't suppport tee).
type sonic.log

REM // Append symbol table to the ROM.
"build_tools\convsym.exe" build_tools\s1built.sym s1built.bin -a -range 0 FFFFFF -inopt "/localSign=." -exclude -filter "(SMPS_(Track|RAM).*)|(.+_(END|End|end))"
